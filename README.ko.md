...
<p align="center">
  <img src="docs/screenshots/dashboard-full.png" alt="DockProbe 대시보드" width="800">
</p>
...
> 호스트 수준 점검에는 볼륨 마운트가 필요합니다 (`/proc:/host_proc:ro`, `/sys:/host_sys:ro`). 마운트되지 않은 경우 해당 점검은 설정 안내와 함께 **사용 불가**로 표시됩니다.
...
- 방화벽에서 포트 허용: `sudo ufw allow 9090/tcp`
- 브라우저에서 자체 서명 인증서 경고 수락
...
1. 공유기에서 9090 포트를 서버의 LAN IP로 포워딩
2. `https://<공인IP>:9090`으로 접속
3. 공인 IP가 바뀌면 DDNS 서비스 (No-IP, DuckDNS 등) 사용
...
**설정 단계:**
1. [Cloudflare Zero Trust](https://one.dash.cloudflare.com)에서 무료 계정 생성
2. **Networks** > **Tunnels** > **Create a tunnel** 이동
3. 터널 이름 지정 (예: `dockprobe`) 후 터널 토큰 복사
4. `bash install.sh` 실행 후 Cloudflare Tunnel 옵션 선택
5. 토큰 입력
6. Cloudflare 대시보드에서 **Public Hostname**을 `http://localhost:9090`으로 설정
7. `https://your-domain.com`으로 정식 TLS 인증서와 함께 접속
...
- **PBKDF2-SHA256** 비밀번호 해싱 — 랜덤 솔트 적용 (600k 반복)
...
- **CSRF 보호** — POST 엔드포인트에 `X-Requested-With` 헤더 필수
- **HTTP 폴백 경고** — 평문 모드 시 시작 시 경고 표시 (터널 전용)

**컨테이너 보안 강화**
- **비루트 실행** — 컨테이너가 `appuser`로 실행, root 아님
...
**프록시 인식 레이트 리밋**
- `X-Forwarded-For`는 `TRUSTED_PROXIES` 환경변수에 등록된 IP에서만 신뢰
- 직접 연결 시 항상 실제 클라이언트 IP 사용
